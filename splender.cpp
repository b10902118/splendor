...
// parameters
constexpr int depth[deckn]{15, 9, 7};
constexpr double rate[deckn]{0.94, 0.88, 0.75};
constexpr double buybase[stage_n] = {6, 8, 10};
// double resbase[stage_n] = {8, 8.5, 9}; // need fix
double resbase = 7;
const double inc_res = 0.3, resbase_max = 11, resbase_fac = 0.6;
const double part = 0.4;
const double dec_jk[3][4] = {
{1.5, 1.8, 2.2, 2.5}, {0.4, 0.7, 1, 1.3}, {0.3, 0.6, 0.9, 1.2}};

constexpr double stage_parm[stage_n][3]{{1.1, 1, 1}, {0.9, 1, 1}, {0.6, 1, 1}};
...
bool iskeycard(card c) {
    ...
    for (int i = 1; i < Gem::normal; ++i) {
        ...
        if (c.cost[i] == 3) three = true;
    }
    if (m == major && c.cost[m] > 5) return true;
    else if (c.cost[m] == 5 && !three) return true;
    return false;
}
...
struct move player_move(struct move m) {
    ...
    // change stage
    if (stage == 0 && my.bns_sum >= 5) stage = 1;
    ...
    for (int lv = 0; lv < 2; ++lv) {
        for (int i = 0; i < table.row; ++i) {
            if (iscard(table[lv][i])) {
                value[lv][i] = score_(table[lv][i]) * stage_parm[stage][lv];

                if (iskeycard(table[lv][i])) value[lv][i] *= majbuf;
                else {
                    value[lv][i] *= buf[table[lv][i].gem];
                }
    ...
    for (int i = 0; i < table.row; ++i) {
        if (iscard(table[2][i])) {
            value[2][i] = score_(table[2][i]) * stage_parm[stage][2] *
    ...
    else if (stage == 1 && major != -1) { // reserve and buy keycards
        for (auto it = card_index.begin(); it != card_index.end(); ++it) {
            card c = table[it->first][it->second];
            int level = it->first;
            if (iskeycard(c)) {
                if (lack(my, c, min(my.gem[Gem::joker], 2 * level)) == 0)
                    return buy(*it);
                else if (reservable(my)) return reserve(*it);
                else {
                    process_step(c);
                }
            }
