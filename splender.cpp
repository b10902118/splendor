...
constexpr double stat_parm[3][4]{
{1.1, 0.9, 1, 1}, {0.8, 1.1, 1, 1}, {0.5, 1, 1, 1.3}};
...
struct move player_move(struct move m) {
    ...
    for (int i = 0; i < table.row; ++i) {
        if (iscard(table[0][i])) {
            ...
            resource[table[0][i].gem].push_back(pair<int, int>(0, i));
        }
        if (iscard(table[1][i])) {
            value[1][i] = score2(table[1][i]);
            card_index.push_back(pair<int, int>(1, i));
            resource[table[1][i].gem].push_back(pair<int, int>(1, i));
        }
        if (iscard(table[2][i])) {
    ...
    for (auto iter = it; iter != card_index.end(); ++iter) {
        card c;
        ...
        if (affordable(my, c)) return buy(*iter);
    }
