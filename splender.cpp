...
double score1(const card &cd) {
    static const double c[6] = {0,       1 * 1,    1.2 * 2,
                                1.3 * 3, 1.45 * 4, 1.55 * 5};
    double score = 0;
    // cout << cd.gem << ": ";
    for (int i : cd.cost) {
...
double score2(const card &cd) {
    static const double c1 = 1.2, c2 = 1.3; // c1*n - c2*pt
    double score = 0;
    // cout << cd.gem << ": ";
    for (int i : cd.cost) {
        // cout << i << ' ';
...
double score3(const card &cd) {
    static const double c1 = 1.2, c2 = 1.3;
    double score = 0;
    for (int i : cd.cost) {
        score += c1 * (i - 1);
    }
...
void init(vector<card> stack_1, vector<card> stack_2,
    ...
    for (int i = 0; i < depth[2]; ++i) {
        keycards[i].gem = stack_3[i].gem;
        keycards[i].score = stack_3[i].score;
        copy();

        if (i < table.row) keycards[i].onboard = true;
        else keycards[i].onboard = false;
    }
